...
The `ConnectorService` interface provides a high-level API for managing platform resources such as assets, missions, tasks, schedulers, organizations, and waypoints. It communicates with the zqnt Connector Service over gRPC and exposes all operations as `CompletableFuture`-based asynchronous calls.
